...
uses
  ...
  FMX.ListView.Adapters.Base, FMX.ListView, BCLsMeetingEvent,
...
procedure gotoPassLock(ALogin: Boolean; AUsername: string = '';
  APassword: string = '');
procedure fnLoadLoading(lo: TLayout; ani: TAniIndicator; stat: Boolean);
function GetMeetingEvent(): TList<TMeetingEvent>;
// ganti ini
...
var
  ...
  username, levelakses, iduser: String;
  statFH, statErr: Boolean;
  WorkerID, coErr, tabCount: Integer; // coErr untuk fnShowE
  FMeetingEvent: TMeetingEvent;

implementation

uses Main, uRest, PassLock, uGoFrame, uSign, ClientModuleApp;
...
Begin
  FMeetingEvent := nil;
  try
    FMeetingEvent := ClientModule.ServerMethods1Client.GetMeetingEvent(WorkerID,
      0, false);
  ...
  begin
    try
      ...
      if Assigned(fraPassLock) then
        fraPassLock.Visible := True
      else
      begin
        fraPassLock := TFraPassLock.Create(frmMain);
        fraPassLock.Parent := frmMain.loLoad;
        fraPassLock.Align := TAlignLayout.Contents;
        fraPassLock.FirstShow;
      end;
    except

    end;

    fraPassLock.PassLock.LearnMode := ALogin;

    fraPassLock.edLUser.text := AUsername;

    if not(ALogin) then
      fraPassLock.PassLock.passvalue := APassword;
    if (AUsername <> '') then
      fraPassLock.edLUser.readOnly := True;
  ...
  begin
    TThread.Synchronize(nil,
      procedure
      begin
        frmMain.loLoad.Visible := false;
        frmMain.aniLoad.Enabled := false;
      end);
  end;
